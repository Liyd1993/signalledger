...
7. For the free AI path, install Ollama, pull `deepseek-r1:7b` and `deepseek-r1:1.5b`, start `ollama serve`, then start `agent/server.py` with `MODEL_PROVIDER=ollama OLLAMA_MODEL_ID=deepseek-r1:7b OLLAMA_STRATEGY_MODEL_ID=deepseek-r1:1.5b`. Set `VITE_AGENT_API_URL=http://127.0.0.1:8787` and repeat steps 3–5. Check `http://127.0.0.1:8787/health` before testing. Tencent Token Plan and Bedrock are optional providers.
