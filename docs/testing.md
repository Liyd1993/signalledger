...
7. For the AI path, set `TENCENT_API_KEY`, start `agent/server.py`, set `VITE_AGENT_API_URL=http://127.0.0.1:8787`, and repeat steps 3–5. Check `http://127.0.0.1:8787/health` before testing. Bedrock is an optional provider, not a requirement for this path.
