...
7. For the free AI path, install Ollama, run `ollama pull qwen2.5:7b`, start `ollama serve`, then start `agent/server.py` with `MODEL_PROVIDER=ollama`. Set `VITE_AGENT_API_URL=http://127.0.0.1:8787` and repeat steps 3–5. Check `http://127.0.0.1:8787/health` before testing. Tencent Token Plan and Bedrock are optional providers.
