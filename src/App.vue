...
function selectTrack(track: AmbientTrack) {
  ...
  selectedTrack.value = track; audioUrl.value = URL.createObjectURL(createWav(track))
}
watch(() => messages.value.length, async () => { await nextTick(); transcript.value?.scrollTo({ top: transcript.value.scrollHeight, behavior: 'smooth' }) })
...
<template>
  <main class="app-shell">
    ...
    <section v-else-if="page === 'reports'" class="list-card" aria-label="全部报告"><button class="back-button" type="button" @click="go('home')">← 返回首页</button><header class="page-title"><p class="eyebrow">YOUR ARCHIVE</p><h1>我的报告</h1><p>每一份都是一次对刚才表达的回顾。</p></header><div v-if="!reports.length" class="empty-archive"><span>✦</span><h2>这里还没有报告</h2><p>完成 10 段文字表达后，就能生成第一份回顾。</p><button class="small-cta" type="button" @click="go('chat')">开始对话</button></div><button v-for="item in reports" v-else :key="item.id" class="archive-item" type="button" @click="showReport(item)"><span>{{ new Date(item.createdAt).toLocaleDateString('zh-CN', { month: 'long', day: 'numeric' }) }}</span><strong>{{ item.title }}</strong><small>{{ item.summary }}</small><b>查看 →</b></button></section>

    <section v-else-if="page === 'audio'" class="audio-card" aria-label="声音陪伴"><button class="back-button" type="button" @click="go('home')">← 返回首页</button><header class="page-title"><p class="eyebrow">SOUND CORNER</p><h1>听一会儿</h1><p>原创的环境声音，给当下留一点安静。</p></header><div class="now-playing"><span>◖</span><div><small>正在选择</small><strong>{{ selectedTrack.title }}</strong><p>{{ selectedTrack.subtitle }}</p></div></div><audio v-if="audioUrl" :src="audioUrl" controls preload="metadata">你的浏览器暂不支持音频播放。</audio><div class="track-list"><button v-for="track in ambientTracks" :key="track.id" type="button" :class="{ active: track.id === selectedTrack.id }" @click="selectTrack(track)"><span>{{ track.kind === 'noise' ? '≈' : '◌' }}</span><div><strong>{{ track.title }}</strong><small>{{ track.subtitle }}</small></div><b>{{ track.id === selectedTrack.id ? '已选择' : '播放' }}</b></button></div><p class="audio-disclaimer">声音用于陪伴与放松，不替代医疗、心理治疗或紧急支持。</p></section>
